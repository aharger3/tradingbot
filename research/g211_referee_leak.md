...
# Second referee, independent pass (same row, different agent)

**Same verdict: refuted.** I sampled a different 10 PNGs (seed 9211 — AVGO_2025-05-02,
BABA_2025-08-28, MU_2025-06-25, MSFT_2025-03-13, NVDA_2025-04-29, AAPL_2024-11-04,
AMD_2024-10-24, NFLX_2026-07-02, MARA_2026-06-02, ACHR_2025-03-17), opened them by eye, and then
ran the three "clean" checks above **over all 100 cards in code** rather than over a sample of 10.
The section above stands unchanged; this adds the full-deck counts.

*(This section was appended after I briefly overwrote the first referee's file. The original text
above is restored verbatim from commit `4b3b8c2e`; nothing of it was lost.)*

| check, run over all 100 cards | count |
|---|---:|
| cards drawing a 1-min candle after the cut | **0 / 100** |
| cards whose cut ≠ his `notes.min` (or ≠ 10:00 when he named none) | **0 / 100** |
| cards whose final 5-min bar is a partial bucket drawn as closed | **91 / 100** |
| overlap in 1-min bar count between the S class and the none class | **0 values** |

The bar-count separation is exact, not approximate: **all 66** none-cards draw exactly **31**
candles; the 34 S-cards draw 5 to 50 and **none of them draws 31**. So the leak survives cropping
the title, and it survives cropping the x-axis labels too — it is in the width of the image.

The first referee's note about tracking is correct and worth repeating, because it is the sharper
operational problem: `git ls-files research/g210_cards` returns **101** files and
`git check-ignore research/g210_cards/index.json` returns nothing. The directory is **tracked, not
gitignored** as `research/g210_render_cards.md` claims, so `index.json` — which carries
`his_grade` for all 100 cards — is committed next to the PNGs a reader would be handed.
