...
- Even the generous upper bound — ANY captured signal, ANY grade (incl. D-skips), every bar — is only S 30/77 = 39%. The engine produces NO signal at ~61% of S bars. (No-dedupe fired-only is also just S 10/77.)
- Of the 789 signals the engine produces (raw, every bar), 653 are downgraded to D and only 117 fire — mostly tight-stop kills (e.g. an S at the OR high with stop $0.22 on a $537 stock). A filter problem sits on top, but it is secondary: even counting every fired bar, S recall is 10/77 = 13%.
...
- S: 28/77, A: 23/60, X: 13/22 — marks with ANY engine signal (incl. D/tight-stop skips, deduped) within +/-2 bars.
- S: 30/77, A: 26/60, X: 15/22 — same but counting EVERY captured signal bar (no dedupe; the true upper bound on 'the engine produced a signal here').
