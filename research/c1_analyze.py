"""C1 BNR_DISPLACEMENT_GATE A/B analyzer: full-pop traded + S>=4+hammer tier
for two backtest_charts.json snapshots (OFF vs ON).

Reuses b4_analyze.py logic verbatim (gstats + tier_sim).
Usage: py -3.13 c1_analyze.py off.json on.json
"""
import json, re, sys
...
if __name__ == "__main__":
    ...
    print_run("GATE OFF (baseline)", ra)
    print_run("GATE ON", rb)
