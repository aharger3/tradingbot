...
**Corrected, pass 2 -> pass 3: it did NOT need a rerun.** Pass 2 said the headline ranking "cannot be repaired without re-running the replay under one exit model held constant" and left it refuted-kept as a second change, out of scope. A third, independent referee pass (`research/r1_referee3.py`, commit `8162c74f`) showed that claim was wrong: the same 12 already-written books can be re-walked, post-hoc, under either exit model held constant across all six arms -- exactly the technique `research/r1_repair.py` already used for the size gate and the honest avg win/loss above, just applied to the exit trigger instead. No replay rerun was needed. See "Refereed (pass 3)" below for the corrected table -- it is now fixed, not kept-refuted.
...
Referee pass 3 (`research/r1_referee.md`, `research/r1_referee3.py`, commit `8162c74f`) reproduced all 84 cells of the 12 stamped books under a THIRD independent implementation (imports none of the builder's or the two earlier referees' code), confirmed `close` against the raw archive tape on 7,857/7,857 rows, confirmed zero lookahead on 120 physically-truncated samples, and reproduced the ACHR entry_idx mismatch candle for candle. It then went further than passes 1-2 and repriced the five `_walk` arms DOWN onto the intrabar-touch stop the shipped engine actually runs, not just `close` UP onto the close-only stop -- something neither earlier pass did. Result: **the headline ranking itself is the exit model.**

`research/r1_referee3.py uniform` (script; re-run here to confirm, same numbers):

| pool | exit model | as_booked | limit_level | next_open | chase_once | close | mid_candle |
|---|---|---:|---:|---:|---:|---:|---:|
| core11 | A: close-only stop (what the five `_walk` arms get) | $479 24/25 | $98 16/25 | **$1,250 22/25** | -$397 10/25 | $1,139 20/25 | $627 20/25 |
| core11 | B: intrabar-touch stop (what the engine gives `close`) | $395 22/25 | -$23 8/25 | **$44 12/25** | -$1,457 3/25 | -$46 13/25 | -$720 7/25 |
| full29 | A: close-only stop (what the five `_walk` arms get) | $832 24/25 | $133 17/25 | **$2,660 23/25** | -$756 8/25 | $2,437 22/25 | $1,108 17/25 |
| full29 | B: intrabar-touch stop (what the engine gives `close`) | $681 24/25 | -$59 8/25 | **$138 13/25** | -$2,661 1/25 | -$221 11/25 | -$1,865 5/25 |

(cells: $/day, green/25 months; unit: every traded signal fired, grade != C, `reentry_84_rule` excluded; fill: the arm; script: `research/r1_referee3.py uniform`)

**The crowned arm's own headline collapses under its own exit model.** `next_open` full29 held to model A (what its published $2,660/day used) reads $2,660/day, 23/25 green; held to model B (the intrabar DISASTER_STOP touch the engine actually runs, the same exit `close` gets) it reads **$138/day, 13/25 green** -- a 95% drop. `core11`: $1,250 -> $44/day, 22/25 -> 12/25 green. Paired 95% CI on next_open-minus-close, held to one model (`research/r1_referee3.py pairci`): model A +0.0140R [+0.0047, +0.0234]; model B +0.0239R [+0.0136, +0.0341] -- **direction survives and separates from zero in both models, magnitude does not**: the published +0.1831R headline edge is roughly seven times either honest figure. Do not read the original two headline tables above (or `next_open`'s $1,250/$2,660 rows in them) as the row's answer; read this table, held to model B, as the operative one -- model B is what the shipped engine's `close` fill actually runs, so it is the fair comparison for every other arm.

**Also fixed in this repair, code-level (not just report prose):**
- `avg_win_loss()` (this script) bucketed by outcome LABEL ("win"/"loss"), which under blind 2R is tautologically +2.0000/-1.0000 by construction and silently dropped every "scratch" row from both columns -- including the -3.9865R / -75.5491R rows named in the "Refereed" section above. Now buckets every filled row by the SIGN of its own R multiple, matching `research/r1_repair.py`'s already-published honest table. A future run of `python research/g210_fill_arms_v2.py --procs 8` will print the honest numbers directly; the already-committed 12 books and the honest table above are unaffected (no rerun performed this repair).
- `book_stamp`'s `SCALE_PLAN` reading in the PARENT process (`main()`) was wrong: `OMEN_SCALE_PLAN=none` is only set inside each spawned WORKER (`run_symbol`), so `backtest_week` is first imported in the parent -- with the env var never set there -- when `write_book()` calls `book_stamp.stamp()`. The parent read `SCALE_PLAN="hod_then_runner_be"` while every worker actually ran with `SCALE_PLAN=None`. Fixed with one line in `main()` (`backtest_week.SCALE_PLAN = None`, forced in the parent too, before any book is stamped). No published number moves -- the workers priced every arm correctly all along; only the stamp's own self-description was wrong. A future rerun's stamps will read `SCALE_PLAN: None`, matching what actually ran; the 12 already-written books keep whatever they stamped and are not rewritten.
- `research/g210_verify.py`'s `limit_level`/`next_open` check only confirmed the booked fill sits inside its own fill bar's `[low, high]` -- true by construction, since `_resting_fill` only ever picks a bar it already found valid, so the check could never fail. It did not confirm the fill bar comes strictly AFTER the signal bar (the actual no-lookahead claim). Added: each of the same 20 sampled rows now also asserts its fill minute is strictly later than its own `entry_time` minute. Re-run: 20/20 pass, 0 lookahead.
...
**Not fixed -- minor, noted only.** Referee pass 3 flagged that `chase_once`'s chase decision uses `max(next.open, next.close)` for a long (the worse-for-the-trade of the two, and it never reaches at or before the signal bar so it is not a lookahead defect), calling it "not a causally clean rule." `chase_once` is last in every ranking under both exit models above regardless, so this does not move a headline; left as-is, inherited from g90, one function outside this row's one-change scope.
