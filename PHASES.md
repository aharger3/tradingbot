...
| **P23** | **The combination — nobody has run it.** P19 (+0.250R, right-signed), P20 (−0.325R, right-signed) and P18 (+0.029R, useless) were each measured **alone, against the committed grader**. The question the ballot actually poses is what they do **together**: all flags on, same 50/50 stratified hold-out, both rigs, reporting recall / false fires / day agreement / the S>A>C ordering against the committed baseline. Note P15 landed nothing, so "the corrected grader" is P19+P20, not a fixed `level_not_respected`. | amber | — |
